...
if($paction=="of_mup_start_convert"){
	...
	while($row=mysql_fetch_assoc($resp)){
		...
		if($row["rowevent"]=="add"){
			$parent = "0";
			if($new_goods){
				$ngrow = __fp_get_row_from_way(  explode("/", $new_goods), "items"  );
				$parent = $ngrow["id"];
			}
			//$parent = "6";
			$addresp = mysql_query("insert into items (recc, tmp, page_show, item_code, parent) values (0, 0, 1, '$mass[0]', $parent)  ");
			$addresp = mysql_query("select * from items order by id desc limit 0,1");
			$addrow = mysql_fetch_assoc($addresp);
			$row["rowevent"]="replace";
			$row["outid"] = $addrow["id"];
		...
		if($row["rowevent"]=="replace"){
			foreach($mass as $key=>$val){
				$noload = false;
				$qmass = __of_up_get_codes_from_index($key, $provider);
				$fname = __of_up_get_name_from_index($key, $provider);
				$val = preg_replace("/'/", "\'", $val);
				$val = preg_replace('/"/', '\"', $val);
				if(is_array($qmass)){
					foreach($qmass as $qkey=>$qval){
						//$qval = preg_replace("/{field}/", "$val", $qval);
...
if($paction=="of_mu_load_file"){
	...
	__of_up_convert_csv_to_db($fillename, $provider);
}
//*************************
if($paction=="of_up_save_sfield"){
	...
	//*********************************************************
	$resp = mysql_query("select * from providers where id=$typeid ");
	...
	echo $tosql;
	$query = "update providers set uploadp_info='$tosql' where id=$typeid";
	$resp = mysql_query($query);
	echo "Ok";
}
//*************************
...
if($paction=="of_up_add_field"){
	...
	$resp = mysql_query("select * from providers where id=$typeid ");
	$row = mysql_fetch_assoc($resp);
	$query = "update providers set uploadp_info='$row[uploadp_info]\n' where id=$typeid";
	$resp = mysql_query($query);
}
//*************************
...
if($paction=="save_template"){
	...
	$query = "update providers set pairs='$code' where id=$tid ";
	$resp = mysql_query($query);
	echo "tid=$tid\n\n$code";
}
//*************************
if($paction=="save_newtype"){
	...
	$query = "insert into providers (name, rus_name, pairs) VALUES ('$neng',  '$nrus', 'saveblock===') ";
	//echo $query;
	$resp = mysql_query($query);
	$resp = mysql_query("select id from providers order by id desc limit 0,1");
	$row = mysql_fetch_assoc($resp);
	echo $row["id"];
}
//*************************
