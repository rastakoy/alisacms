...
if($_FILES["qqfile"]){
	...
	if($res) {
		$query = "INSERT INTO images (name, parent, link, prior, img_in_text) VALUES ('img', $parent, '$filenames', 0, 1)";
		$resp = mysql_query($query);
...
if($aa){
	...
	if($row["parent"]){
		...
		//echo "переименовываем ../loadimages/$row[link]<br/>\n";
		...
		if(file_exists("../loadimages/".$srow["href_name"]."_.jpg")){
			//echo "Файл ../loadimages/".$srow["href_name"]."_.jpg существует<br/>\n";
		}
