			...
			if(mysql_num_rows($resp) > 0){
				...
				if($foldernum){
					...
					if($wwrow["name"]){
						...
						//$respo = mysql_query("select * from items where id=$row_art[parent]");
						//$rowo = mysql_fetch_assoc($respo);
						//if($rowo["id"] != $foldernum){
						//	$queryo = "update items SET  parent= where id=$row_art[id]   ";
						//	$respo = mysql_query("select * from items where id=$row_art[parent]");
						}
					}
				} else {
