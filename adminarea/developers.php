...
if($devname){
	$resp = mysql_query("INSERT INTO developers (name) VALUES ('$devname')");
}
?>
<html>
...
echo "<div class=\"ui-state-default-3\" id=\"myitems_sortable\">";
	$resp = mysql_query("select * from developers order by name asc");
	while($row=mysql_fetch_assoc($resp)){
		...
		<span class=\"span_fast_name\" id=\"csfn_1_$row[id]\">$row[country]&nbsp;</span>
		<span class=\"span_fast_name\" id=\"csfn_2_$row[id]\">$row[artic]&nbsp;</span>
		</div>";
		echo "</div>";
	}
echo "</div>";
