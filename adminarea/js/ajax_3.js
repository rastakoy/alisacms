		...
		$.ajax({
			...
			success: function(html) {
				...
				for(var i=0; i<json_length(post_object); i++){
					...
					} else if(post_object[i][0] == "name"){
						npaction += "&" + post_object[i][0] + "=" + document.getElementById('newsname').value.replace(/&/gi, "~~~aspirand~~~");
					} else if(post_object[i][2] == "list"){
						mobj = document.getElementById(post_object[i][1]);
						mobjs = mobj.getElementsByTagName("option");
						oobj_txt = "";
						for(moj=0; moj<mobjs.length; moj++){
							oobj = mobjs[moj];
							if(oobj.selected) oobj_txt += oobj.value+"~";
