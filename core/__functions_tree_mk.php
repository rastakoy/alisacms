...
function __ft_search_first_data($table, $id, $field){
	...
	$query = "select $field from $table where folder=0 && parent=$id && $field!='' order by prior asc limit 0,1";
...
function __farmmed_rekursiya_show_items_user($all_parent, $edit_mass, $edit, $count, $most_show=false){
	...
	while($row=mysql_fetch_assoc($resp)){
			...
			//echo "count=$count<br />";
			$ret_val.= "<div";
			if(__marabu_tree_test_for_child($row["id"])   ||   $all_parent==0) {
				if($row["parent"]==0){
					$ret_val.= "  >";
				}
				else { 
			...
			if(__marabu_tree_test_for_child($row["id"]) || $all_parent==0) {
					...
					//for($i=0; $i<$count-1; $i++) $ret_val.= "<td width=\"13\">&nbsp;</td>\n";
						...
						$ret_val.= "	<a href=\"javascript:hide_tree_item($row[id], $count)\">
									<img src=\"/tree/minus.jpg\" class=\"tree-img\"></a>";
						//$ret_val.= "</td>\n";
						}
						else{
						//$ret_val.= "<td width=\"13\"  id=\"tree_plus_$row[id]\">";
						$ret_val.= "<a href=\"javascript:show_tree_item($row[id], $count)\">
									<img src=\"/tree/plus.jpg\" class=\"tree-img\"></a>";
						//$ret_val.= "</td>\n";
						}
					}
				}
					...
					if($all_parent!=0) $ret_val.= "\n
														...
														<img src=\"/tree/for_x_for.gif\" class=\"tree-img\">\n";
					else
						 if(!$tstm[0]) $ret_val .= "";
						 else $ret_val.= "
														<img width=\"12\" src=\"/tree/spacer.gif\" class=\"tree-img\">\n";
			...
			if($row["parent"]==0){
				...
				$ret_val.= "";
								...
								//<tr>";
								//$ret_val .= "<td class=\"td_gr_menu\">";
								if(!$tstm[0]) {
									//$ret_val .= "<a href=\"items.php?parent=$row[id]\"  style=\"color: #333333;\">";
									if($most_show[0]==$row["id"])
										$ret_val .= create_template_menu(true, $row["name"], "items.php?parent=$row[id]");
									else
										$ret_val .= create_template_menu(false, $row["name"], "items.php?parent=$row[id]");
								}
								else {
									//$ret_val .= "<a href=\"javascript:show_tree_item($row[id], $count)\"  style=\"color: #333333;\">";
									if($most_show[0]==$row["id"])
										$ret_val .= create_template_menu(true, $row["name"], "javascript:show_tree_item($row[id], $count)");
									else
										$ret_val .= create_template_menu(false, $row["name"], "javascript:show_tree_item($row[id], $count)");
								}
								//************* Название здесь: ***************************
			...
			//***********************************
				if(__marabu_tree_test_for_child($row["id"])){
					if($row["id"] == $most_show[0]){
						$ret_val .= "<div id=\"is_open_$row[id]\" style=\"display:; padding-bottom:10px;";
						...
						$ret_val .= "<div id=\"is_open_$row[id]\" style=\"display:none; padding-bottom:10px;";
						if($count==0) $ret_val.="padding-left:25px;";
						$ret_val .= "\"><img width=\"16\" height=\"16\" src=\"/tree/clock.gif\" align=\"absmiddle\">Торговое оборудование, Киев";
					}
					$ret_val .= "</div>";
				}
			//***********************************
...
//*******************************************
//*****************************************************
function __farmmed_rekursiya_show_items_v2_($db_table, $all_parent, $edit_mass, $edit, $count, $most_show=false){
	$ret_val="";
	if($all_parent==0){
		//$ret_val.= "<div class=\"tree-spacer\"></div>\n";
		$ret_val.= "<div id=\"tree_folder_0_0\" class=\"tree-root-folder\" onMouseOver=\"show_drag_folder(this);\"></div>\n";
		$ret_val.= "<div class=\"tree-content\"><a href=\"?parent=0&imgs=1\">Корневая группа</a></div>\n"; 
	}
	$query = "select * from ".__tree_get_tree_name_from_index($db_table)." where parent=$all_parent and folder=1 ORDER BY prior, name ASC";
	//echo $query;
	$resp=mysql_query($query);
	while($row=mysql_fetch_assoc($resp)){
		//if($edit_mass["id"]!=$row["id"]){
			...
			if(!$user){
				$ret_val.="<div onMouseOver=\"f_show_edit_folder($row[id]);hide_drag_folder()\" onMouseOut=\"f_hide_edit_folder($row[id])\">\n";
				$ret_val.= "<div class=\"tree-content\"><a href=\"?parent=$row[id]&imgs=1\">$row[name]</a></div>\n"; 
				$ret_val.= "<div id=\"show_edit_folder_$row[id]\" style=\"display: none;\">";
				$ret_val.= "<div class=\"tree-new\">
								<a title=\"Добавить группу\" href=\"?new_folder=1&parent=$row[id]\">
								<img class=\"tree-image\" src=\"tree/new_item.gif\" border=\"0\" alt=\"Новая подгруппа\" /></a>
								</div>\n";
				$ret_val.= "<div class=\"tree-edit\">
								<a title=\"Редактировать группу\" href=\"?edit_folder=$row[id]&parent=$row[id]\">
	...
	//***********************************/
				if(__marabu_tree_test_for_child($row["id"], $db_table)){
					...
					if($row["id"] == $most_show[0]){
						...
						for($i=1; $i<count($most_show); $i++)
							$new_most_show[] = $most_show[$i];
					$ret_val.= __farmmed_rekursiya_show_items_v2_($db_table, $row["id"], $edit_mass, $edit, $count+1, $new_most_show);
					}
					else{
						$ret_val .= "<div id=\"is_open_".$db_table."_$row[id]\" style=\"display:none;\">";
						$ret_val .= "<img width=\"16\" height=\"16\" src=\"tree/clock.gif\" align=\"absmiddle\">Загрузка...";
