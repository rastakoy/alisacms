...
//fputs($smtp_conn,base64_encode("pT2R0lggVQsi")."\r\n");
$code = substr(get_data($smtp_conn),0,3);
...
$header.="X-Priority: 3 (Normal)\r\n";
$header.="Message-ID: <172562218.".date("YmjHis")."@mysecret.kh.ua\r\n";
...
if($user["paytype"] == 1) $mypaytype = "Предоплата";
else $mypaytype = "Наложенный платеж";
...
Условие доставки: $mypaytype
Комментарии: $user[comment]

Заказ ";

$nu_uszakaz =  $user["paytype"];
...
$smtp_conn = fsockopen("mail.mysecret.kh.ua", 25,$errno, $errstr, 10);
if(!$smtp_conn) {print "соединение с серверов не прошло"; fclose($smtp_conn); $stop_send = true;}
if(!$stop_send){
$data = get_data($smtp_conn);
fputs($smtp_conn,"EHLO mysecret.kh.ua\r\n");
$code = substr(get_data($smtp_conn),0,3);
if($code != 250) {print "ошибка приветсвия EHLO"; fclose($smtp_conn); $stop_send = true;}
}
...
if(!$stop_send){
fputs($smtp_conn,base64_encode("robot@mysecret.kh.ua")."\r\n");
$code = substr(get_data($smtp_conn),0,3);
if($code != 334) {print "ошибка доступа к такому юзеру"; fclose($smtp_conn); $stop_send = true;}
}
...
if(!$stop_send){
fputs($smtp_conn,"MAIL FROM:robot@mysecret.kh.ua\r\n");
$code = substr(get_data($smtp_conn),0,3);
...
//fputs($smtp_conn,"RCPT TO: robot@mysecret.kh.ua\r\n");
$code = substr(get_data($smtp_conn),0,3);
...
if(!$stop_send){
	...
	if(!$stop_send) {
		...
		//sleep(10);
		//__fpost_send_mail("Сайт Гардена", $user["email"], $user["fio"], "info@mysecret.kh.ua", "Счет на оплату заказа", 
		//$cont, "mysecret.kh.ua", "robot@mysecret.kh.ua", "pT2R0lggVQsi", 1 );
	...
	color: #333333;
		...
		//print_r($user);
		//echo $cont;
		__fpost_send_mail("Сайт mysecret.kh.ua", 
		$user["email"], 
		$user["fio"], 
		"robot@mysecret.kh.ua", 
		"Заказ с интернет-магазина mysecret.kh.ua", 
		$cont, 
		"mysecret.kh.ua", 
		"robot@mysecret.kh.ua", 
		"pT2R0lggVQsi", 1);
