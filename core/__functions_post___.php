...
//fputs($smtp_conn,base64_encode("g!?3kAWbTok+")."\r\n");
$code = substr(get_data($smtp_conn),0,3);
...
$code = substr(get_data($smtp_conn),0,3);
if($code != 250 AND $code != 251) {print "Сервер не принял команду RCPT TO (непонятно куда) (".$to.")"; fclose($smtp_conn); $stop_send = true;}
}
...
//*******************************************************************************
//*******************************************************************************
function __fp_post_zakaz($user, $nu_phone=false, $nu_email=false, $nu_fio=false, 
$nu_place=false, $nu_uszakaz=false, $nu_city=false){
//**********************************************
//echo $nu_phone."::".$nu_email."::".$nu_fio."::".$nu_place."<br><br>";
...
Телефон: $user[phone]
E-mail: $user[email]
Место доставки: $user[place]
"; 
$mytm = mktime (date("G") ,date("i") ,date("s") , date("m")  ,date("d"),date("Y"));
$text .= strftime("%d.%m.%Y %H:%M:%S", $mytm ); $text.="
...
for($i=0; $i<count($re_mass)-1; $i++){
	...
	$mass["$tm[0]"] = $tm;
			...
			$text.= "\n+------------------------------------------------------------------------------------------------------+\n";
				...
				$text.= "   Артикул: ".$row["model_article"]."\n";
...
$code = substr(get_data($smtp_conn),0,3);
if($code != 250 AND $code != 251) {print "Сервер не принял команду RCPT TO (отправка заказа)"; fclose($smtp_conn); $stop_send = true;}
}
...
if(!$stop_send){
	...
	if(!$stop_send) {
		...
		//sleep(10);
		//__fpost_send_mail("Сайт Гардена", $user["email"], $user["fio"], "info@sad-dom.kiev.ua", "Счет на оплату заказа", 
		//$cont, "sad-dom.kiev.ua", "robot@sad-dom.kiev.ua", "g!?3kAWbTok+", 1 );
	...
	color: #333333;
		...
		<body>";
		if($nu_phone && $nu_email && $nu_fio && $nu_place)
			$cont.=__fpost_zakaz_html($num_zak, $nu_phone, $nu_email, $nu_fio, $nu_place);
		else 
			$cont.=__fpost_zakaz_html($num_zak);
		$cont .= "</body></html>";
		//echo $cont;
		//print_r($user); echo "TEST TEST TEST";
		__fpost_send_mail("Сайт Поклевка", $user["email"], $user["fio"], "robot@sad-dom.kiev.ua", "Счет на оплату заказа", 		$cont, "poklevka.net.ua", "robot@poklevka.net.ua", "1y6xQX9rhy1i", 1);
...
function __fpost_zakaz_html($cur_id, $nu_phone=false, $nu_email=false, $nu_fio=false, $nu_place=false){
	...
	if(!$row_user) {
		...
		//$a = explode(":", $user[0]);
		//$row_user["phone"] = $a[0];
		//$row_user["email"] = explode(":", $user[0]);
		//$row_user["email"] = $row_user["email"][0];
		//$row_user["place"] = $nu_place;
		//$row_user["id"] = "0";
	}
	//print_r($row_user);
  ...
  <tr>
    <td colspan=2><font color=red><b>Перед оплатой чека уточните наличие товара у продавца</b></font></td>
  </tr>
</table>
