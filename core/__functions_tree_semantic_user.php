...
function get_item_content($id){
	...
	$ret .= "<hr size=\"1\"></div><div id=\"div_item_content\" style=\"height:300px; overflow: auto;\">$row[cont]</div>";
	$ret .= "<textarea name=\"item_cont\" cols=\"65\" rows=\"7\" id=\"item_cont\" style=\"display:none;\">$row[cont]</textarea>";
	return $ret;
}
//*****************************************************
function show_images_for_items_sort($id){
	$ret = "";
...
//*****************************************************
function show_images_for_items($id){
...
function __farmmed_rekursiya_show_semantik_tree($db_table, $all_parent, $edit_mass, $edit, $count, $most_show=false){
	$ret_val="";
	$query="select * from ".__tree_get_tree_name_from_index($db_table)." where parent=$all_parent and folder=1 && tmp!=1 && recc!=1 && page_show=1 ORDER BY prior, name ASC";
	//echo $query."<br/>\n";
	$resp=mysql_query($query);
	...
	while($row=mysql_fetch_assoc($resp)){
			...
			if(__marabu_tree_test_for_child($row["id"], $db_table)) {
					...
					//<img src=\"tree/minus.jpg\" align=\"absmiddle\" border=\"0\" ></a>";
				}
				else{
					//$ret_val.= "<a href=\"javascript:show_li_item($db_table, $row[id])\">
					//<img src=\"tree/plus.jpg\" align=\"absmiddle\" border=\"0\" ></a>";
				}
			}
			else {
				//if($all_parent!=0) {
					if($edit==$row["id"])
						$ret_val.= "<img src=\"tree/4x4_red.gif\" align=\"absmiddle\" border=\"0\" >";
					//else
					//	$ret_val.= "<img src=\"tree/4x4_blue.gif\" align=\"absmiddle\" border=\"0\" >";
				//}
			}
			$ret_val.= "<a href=\"".__fp_create_folder_way(__tree_get_tree_name_from_index($db_table), $row["id"], 1)."\">$row[name]</a>";
...
//*******************************************
//*****************************************************
function __fts_tree_start($db_table, $all_parent, $edit_mass, $edit, $count, $most_show=false){
	$ret_val="";
	//echo "asd test"; print_r($most_show);
	$query="select * from ".__tree_get_tree_name_from_index($db_table)." where 
	parent=$all_parent && folder=1 && page_show=1 ORDER BY prior, name ASC";
			...
			//***********************************
				if(__marabu_tree_test_for_child($row["id"], $db_table)){
					if($row["id"] == $most_show[0]){
						...
						for($i=1; $i<count($most_show); $i++)
							$new_most_show[] = $most_show[$i];
						$ret_val.= __fts_tree_start($db_table, $row["id"], $edit_mass, $edit, $count+1, $new_most_show);
						//$ret_val .= "</ul>";
					}
...
function __farmmed_rekursiya_show_semantik_tree_json($db_table, $all_parent, $edit_mass, $edit, $count, $most_show=false){
	global $global_brend, $global_brend_link;
	$gb = $global_brend;
	$gbl = $global_brend_link;
	$ret_val="";
	$query="select * from ".__tree_get_tree_name_from_index($db_table)." 
	where parent=$all_parent and folder=1 && page_show=1 ORDER BY prior, name ASC";
	$resp=mysql_query($query);
	$tabs = "";
	for($i=0; $i<$count; $i++) $tabs.= "	";
	$ret_val.= "$tabs{\n";
	...
	while($row=mysql_fetch_assoc($resp)){
			...
			$ret_val.= "$tabs		\"link\": \"".$gbl.__fp_create_folder_way(__tree_get_tree_name_from_index($db_table), $row["id"], 1)."\",\n";
			if($edit){
...
function __fmt_rekursiya_show_items_for_select($db_table, $all_parent, $edit_mass, $edit, $count, $most_sel=0){
	...
	$ret_val="";
	$resp=mysql_query("select * from ".__tree_get_tree_name_from_index($db_table)." where parent=$all_parent and folder=1 and tmp!=1 order by prior asc, name asc");
	while($row=mysql_fetch_assoc($resp)){
