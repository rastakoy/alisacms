...
//*******************************************************************************
//*******************************************************************************
function __fp_post_zakaz($user, $nu_phone=false, $nu_email=false, $nu_fio=false, $nu_place=false){
//**********************************************
...
for($i=0; $i<count($re_mass)-1; $i++){
	...
	$mass["$tm[0]"] = $tm;
			...
			$text.= "\n+------------------------------------------------------------------------------------------------------+\n";
				...
				$text.= "   Цена: ".__format_txt_price_format($row["model_price"])."\n";
...
if(!$stop_send){
	...
	if(!$stop_send) {
		...
		//sleep(10);
		//__fpost_send_mail("Сайт Гардена", $user["email"], $user["fio"], "info@gardena-shop.com.ua", "Счет на оплату заказа", 
	...
	color: #333333;
		...
		__fpost_send_mail("Сайт Гардена", $user["email"], $user["fio"], "robot@gardena-shop.com.ua", "Счет на оплату заказа", 		$cont, "gardena-shop.com.ua", "robot@gardena-shop.com.ua", "njr48KJeU&Y89lkg", 1);
